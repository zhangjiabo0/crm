...
class PHPMailer{
            ...
            for($e=0;$e<count($line_part);$e++){
                ...
                if($qp_mode and (strlen($word)>$length)){
                    ...
                    if(strlen($buf)>$length and $buf_o!=''){
                        $message.=$buf_o.$soft_break;$buf=$word;
                    }
                }
            }
            $message.=$buf.$this->LE;
    ...
    public function EncodeHeader($str,$position='text'){
        ...
        switch(strtolower($position)){
            ...
            case 'text':
            default:
                $x+=preg_match_all('/[\000-\010\013\014\016-\037\177-\377]/',$str,$matches);break;
        }
        if($x==0){
            return ($str);
        }
    ...
    public function Base64EncodeWrapMB($str){
        ...
        for($i=0;$i<$mb_length;$i+=$offset){
            ...
            do{
                ...
                $chunk=base64_encode($chunk);$lookBack++;
...
class SMTP{
    ...
    public function Connect($host,$port=0,$tval=30){
        ...
        stream_set_blocking($this->smtp_conn, true); //重要，设置为非阻塞模式
        stream_set_timeout($this->smtp_conn,10); //设置超时
    ...
    public function Data($msg_data){
        ...
        if($code!=250){
            ...
            alert('error','error:DATA not accepted from server,smtp_code:'.$code.',smtp_msg:'.substr($rply,4));
            if($this->do_debug>=1){
                echo "SMTP -> ERROR: ".$this->error["error"].": ".$rply.$this->CRLF.'<br />';
            }
            return false;
