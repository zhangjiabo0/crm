...
class LeadsBAction extends CommonAction{
				...
				if($m_leadsB_data->create()!==false){
					if($_POST['contactsB_name']){
						...
						if(!empty($contactsB)){
							...
							$contactsB['owner_role_id'] = session('role_id');
	...
	public function index(){
		...
		switch ($by) {
			...
			default : $where['owner_role_id'] = array('in',implode(',', getSubRoleIdByYuan())); break;
		}
		if ($by != 'deleted') {
...
// 		die;
		if (!isset($where['owner_role_id'])) {
			if(!isset($where['_string'])) $where['owner_role_id'] = array('in', implode(',', getSubRoleIdByYuan(true)));
			else $where['owner_role_id'] = array('in', '0,'.implode(',', getSubRoleIdByYuan(true)));
		}
		...
		if(trim($_GET['act'] == 'sms')){
			...
			if($by == 'deleted') {
				foreach ($list as $k => $v) {
					...
					$list[$k]["owner"]["Dept_3_Name"] = getDept_3_Name($list[$k]["owner"]["role_id"]);
				}
