...
// 			$this->display('Setting:sendsms');
			...
			if(vali_permission('leads', 'export')){
				...
				$leadsList = $d_v_leads->where($where)->order($order)->limit($limit)->select();
				session('export_status', 1);
				$this->ajaxReturn(array('list'=>$leadsList,'count'=>count($leadsList)), '', 1);
// 				$this->excelExport($leadsList);
			}else{
...
// 			$this->display();
	...
	public function excelExport($leadsList=false){
		...
		$objProps->setTitle("crm Leads Data");    
		$objProps->setSubject("crm Leads Data");    
		$objProps->setDescription("crm Leads Data");    
		$objProps->setKeywords("crm Leads Data");    
        ...
        header("Content-Disposition:attachment;filename=crm_leads_".date('Y-m-d',mktime())."_".$current_page.".xls");
	...
	public function getCurrentStatus(){
 	...
 	public function excelImportDownload(){
		...
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		header("Content-Type: application/vnd.ms-excel;");
        header("Content-Disposition:attachment;filename=crm_leads.xls");
        header("Pragma:no-cache");
        header("Expires:0");
        $objWriter->save('php://output'); 
    }
