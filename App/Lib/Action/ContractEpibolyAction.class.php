<?php 
class ContractEpibolyAction extends CommonAction {
	...
	public function add(){
		...
		if(!$contract_epiboly_custom)  $contract_epiboly_custom = 'FWWB';
		if($this->isPost()){
			$contract_epiboly = M('contractEpiboly');
...
// 			$data['customer_id'] = intval($_POST['customer_id']);
			...
			list($data['confirm'],$data['confirm_name']) = getContractEpibolyFlow(session('role_id'));
			if($contractId = $contract_epiboly->add($data)){
				//在price_sheet中加上标识表示已转服务合同
				M('PriceSheet')->where(array('id'=>intval($_POST['price_sheet_id'])))->save(array('t_epiboly'=>1));
...
// 				M('RBusinessContract')->add(array('contract_id'=>$contractId,'business_id'=>$data['business_id']));
				...
				if($_POST['refer_url']){
					...
					alert('success', L('CREATE_A_CONTRACT_SUCCESSFULLY'), U('contractEpiboly/index'));
				}
			}else{
				alert('error', L('FAILED_TO_CREATE_THE_CONTRACT'), U('contractEpiboly/add?price_sheet_id='.intval($_POST['price_sheet_id'])));
			}
		}else{
			if(intval($_GET['price_sheet_id'])){
				...
				$this->assign('contract_custom', $contract_epiboly_custom.date('Ymd').$num_str);
	...
	public function edit(){
		...
		if($_GET['type'] == 'confirm'){
			$flow_log = M('ContractEpibolyFlowLog')->where(array('role_id'=>session('role_id'),'contract_flow_id'=>$contract_id,'_string'=>'result is null'))->find();
			if(empty($flow_log)){
			...
			if(!check_permission($contract_id, 'contractEpiboly')) $this->error(L('HAVE NOT PRIVILEGES'));
		}
			
		$contract_epiboly_info = $contract_epiboly->where('contractEpiboly.contract_id = %d',$contract_id)->find();
		$product = M('RPriceSheetProduct')->where(array('sheet_id'=>$contract_epiboly_info['price_sheet_id']))->select();
		$this->assign('price_sheet_id',$contract_epiboly_info['price_sheet_id']);
		if (is_array($contract_epiboly_info)) {
			if($_POST['submit']){
				$data['due_time'] = $_POST['due_time']?strtotime($_POST['due_time']):time();
				...
				if(M('contractEpiboly')->where(array('contract_id'=>$contract_id))->save($data)){
					$confirm_array = array_filter(explode('|',$data['confirm']));
					if(!empty($confirm_array[0])){
						...
						$flow_data['role_id'] = M('User')->where(array('name'=>$confirm_array[0]))->getField('role_id');
						$flow_data['user_name'] = M('User')->where(array('name'=>$confirm_array[0]))->getField('true_name');
						$last_step = M('ContractEpibolyFlowLog')->where(array('contract_flow_id'=>$contract_id))->order('step desc')->limit(1)->getField('step');
...
// 					M('rBusinessContract')->where(array('contract_id'=>$contract_id))->save(array('business_id'=>$data['business_id']));
					alert('success', L('MODIFY_THE_SUCCESS'),U('contractEpiboly/view','id='.$contract_id));
				}else{
					alert('success', L('THERE_WERE_NO_CHANGES_IN_DATA'),$_SERVER['HTTP_REFERER']);
				}
			}else{
				$this->assign('info',$contract_epiboly_info);
				$this->assign('product',$product);
				$this->alert = parseAlert();
				$this->display();
			}
	...
	public function mark(){
		...
		//判断是否可以审核
		$ContractEpibolyFlowLog = M('ContractEpibolyFlowLog')->where(array('role_id'=>$role_id,'contract_flow_id'=>$contract_id,'_string'=>'result is null'))->find();
		//已走的最后一个拒绝
		$flow_log_last_result_0 = M('ContractEpibolyFlowLog')->where(array('contract_flow_id'=>$contract_id,'result'=>0))->order('step desc')->limit(1)->find();
		$flow_log_last_result_0['step'] = $flow_log_last_result_0['step']?$flow_log_last_result_0['step']:20;
		
		$is_cancel = M('ContractEpiboly')->where(array('contract_id'=>$contract_id))->getField('is_cancel');
		if(empty($ContractEpibolyFlowLog) || $is_cancel == 1){
...
// 		$data['user_name'] = M('User')->where(array('role_id'=>$role_id))->getField('true_name');
		...
		if($type == 'approve'){
			$data['result'] = 1;
			$res = M('ContractEpibolyFlowLog')->where(array('id'=>$ContractEpibolyFlowLog['id']))->save($data);
			if($res){
				//下一步
				$confirm = M('ContractEpiboly')->where(array('contract_id'=>$contract_id))->getField('confirm');
				$confirm = array_filter(explode('|',$confirm));
				$emp_no = M('User')->where(array('role_id'=>$role_id))->getField('name');
// 				$i = array_search($emp_no,$confirm);
				$i = $ContractEpibolyFlowLog['step']-$flow_log_last_result_0['step']-1;
	...
	public function cancel(){
		...
		$contract_epiboly = M('ContractEpiboly')->where(array('contract_id'=>$contract_id))->find();
		if(empty($contract_epiboly) || $contract_epiboly['owner_role_id']!=session('role_id') || $contract_epiboly['is_cancel'] == 1){
			$this->error(L('CAN NOT CANCEL'));
		}
		$res = M('ContractEpiboly')->where(array('contract_id'=>$contract_id))->save(array('is_cancel'=>1,'cancel_time'=>time()));
		if($res){
			alert('success', L('CANCEL SUCCESS'),U('contractEpiboly/index'));
		}else{
			alert('error', L('CANCEL FAILED'),U('contractEpiboly/index'));
		}
	}
...
// 		if(!check_permission($contract_id, 'contract')) $this->error(L('HAVE NOT PRIVILEGES'));
		$contract_epiboly = D('ContractEpibolyView');
		if (0 == $contract_id) alert('error', L('NOT CHOOSE ANY'), U('contractEpiboly/index'));
		
		$info = $contract_epiboly->where(array('contract_id'=>$contract_id))->find();
		
		$all_ids = getSubRoleIdByYuan(true);
		if(!in_array($info['owner_role_id'],$all_ids)) $this->error(L('HAVE NOT PRIVILEGES'));
		if(empty($info)) alert('error', L('THE_CONTRACT_DOES_NOT_EXIST_OR_HAS_BEEN_DELETED'), U('contractEpiboly/index'));
		$info['creator_name'] = M('user')->where('role_id = %d', $info['creator_role_id'])->getField('name');
		
		//检验是否可以编辑
		$ContractEpibolyFlowLogLast = M('ContractEpibolyFlowLog')->where(array('contract_flow_id'=>$contract_id))->order('step desc')->limit(1)->find();
		if($ContractEpibolyFlowLogLast['result'] === '0' && $info['owner_role_id'] == session('role_id')){
		...
		if($flow_log_last['result'] === '1'){
			...
			foreach ($flow_log_should as $k=>$v){
				$user = D('UserView')->where(array('name'=>$v))->find();
				if($v == $flow_log_last['emp_no'] && $k == $flow_log_last['step']-$flow_log_last_result_0['step']-1){
		...
		$file_ids = M('rContractEpibolyFile')->where('contract_id = %d', $contract_id)->getField('file_id', true);
	...
	public function index(){
		...
		$all_ids = getSubRoleIdByYuan(true);
		$where = array();
		$order = 'contractEpiboly.create_time desc';
		if($_GET['desc_order']){
		...
		switch ($_GET['by']){
			case 'create':
				$where['contractEpiboly.creator_role_id'] = session('role_id');
				break;
			case 'sub' : 
				$where['contractEpiboly.owner_role_id'] = array('in',implode(',', $below_ids)); 
				break;
			case 'subcreate' : 
				$where['contractEpiboly.creator_role_id'] = array('in',implode(',', $below_ids)); 
				break;
			case 'today' : 
				$where['contractEpiboly.due_time'] =  array('between',array(strtotime(date('Y-m-d')) -1 ,strtotime(date('Y-m-d')) + 86400)); 
				break;
			case 'week' : 
				$week = (date('w') == 0)?7:date('w');
				$where['contractEpiboly.due_time'] =  array('between',array(strtotime(date('Y-m-d')) - ($week-1) * 86400 -1 ,strtotime(date('Y-m-d')) + (8-$week) * 86400)); 
				break;
			case 'month' : 
				$next_year = date('Y')+1;
				$next_month = date('m')+1;
				$month_time = date('m') ==12 ? strtotime($next_year.'-01-01') : strtotime(date('Y').'-'.$next_month.'-01');
				$where['contractEpiboly.due_time'] = array('between',array(strtotime(date('Y-m-01')) -1 ,$month_time));
				break;
			case 'add' : 
				$order = 'contractEpiboly.create_time desc'; 
				break;
			case 'deleted' : 
				$where['contractEpiboly.is_deleted'] = 1; 
				break;
			case 'update' : 
				$order = 'contractEpiboly.update_time desc'; 
				break;
			case 'me' :
				$where['contractEpiboly.owner_role_id'] = session('role_id');
				break;
			default: 
				$where['contractEpiboly.owner_role_id'] = array('in',implode(',', $all_ids)); 
				break;
		}
		
		if (!isset($where['contractEpiboly.is_deleted'])) {
			$where['contractEpiboly.is_deleted'] = 0;
		}
		if (!isset($where['contractEpiboly.owner_role_id'])) {
			$where['contractEpiboly.owner_role_id'] = array('in',implode(',', $all_ids)); 
		}
			...
			switch ($condition) {
				case "is" : 
					if($field == 'customer_id'){
						...
						$where['contractEpiboly.'.$field] = array('eq',$search);
					}break;
				case "isnot" :  $where['contractEpiboly.'.$field] = array('neq',$search);break;
				case "contains" :  $where['contractEpiboly.'.$field] = array('like','%'.$search.'%');break;
				case "not_contain" :  $where['contractEpiboly.'.$field] = array('notlike','%'.$search.'%');break;
				case "start_with" :  $where['contractEpiboly.'.$field] = array('like',$search.'%');break;
				case "end_with" :  $where['contractEpiboly.'.$field] = array('like','%'.$search);break;
				case "is_empty" :  $where['contractEpiboly.'.$field] = array('eq','');break;
				case "is_not_empty" :  $where['contractEpiboly.'.$field] = array('neq','');break;
				case "gt" :  $where['contractEpiboly.'.$field] = array('gt',$search);break;
				case "egt" :  $where['contractEpiboly.'.$field] = array('egt',$search);break;
				case "lt" :  $where['contractEpiboly.'.$field] = array('lt',$search);break;
				case "elt" :  $where['contractEpiboly.'.$field] = array('elt',$search);break;
				case "eq" : $where['contractEpiboly.'.$field] = array('eq',$search);break;
				case "neq" : $where['contractEpiboly.'.$field] = array('neq',$search);break;
				case "between" : $where['contractEpiboly.'.$field] = array('between',array($search-1,$search+86400));break;
				case "nbetween" : $where['contractEpiboly.'.$field] = array('not between',array($search,$search+86399));break;
				case "tgt" :  $where['contractEpiboly.'.$field] = array('gt',$search+86400);break;
		...
		$list = $contract_epiboly->where($where)->page($p.','.$listrows)->order($order)->select();
		$count = $contract_epiboly->where($where)->count();
...
// 			$list[$key]['contacts_name'] = M('contacts')->where('contacts_id = %d', $contacts_id)->getField('name');
			$end_date =  $contract_epiboly->where('contract_id = %d', $value['contract_id'])->getField('end_date');
			if($end_date){
				$list[$key]['days'] = floor(($end_date-time())/86400+1);
			}
			$ContractEpibolyFlowLogToMe = M('ContractEpibolyFlowLog')->where(array('role_id'=>session('role_id'),'contract_flow_id'=>$value['contract_id'],'_string'=>'result is null'))->find();
			if($ContractEpibolyFlowLogToMe && $value['is_cancel'] == 0){
				$list[$key]['confirm_to_me'] = true;
			}else{
				$list[$key]['confirm_to_me'] = false;
			}
			$ContractEpibolyFlowLogLast = M('ContractEpibolyFlowLog')->where(array('contract_flow_id'=>$value['contract_id']))->order('step desc')->limit(1)->find();
			if($ContractEpibolyFlowLogLast['result'] === '0' && $value['owner_role_id'] == session('role_id')){
...
// 		$this->display();
	...
	public function getcontractepibolylist(){
		$contract_epiboly = D('ContractEpibolyView');
		$list = $contract_epiboly->where(array('contract_epiboly.is_deleted' => 0))->select();
