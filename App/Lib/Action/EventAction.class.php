...
class EventAction extends Action{
	public function _initialize(){
		$action = array(
			...
			'allow'=>array('close','open','getcurrentstatus')
		);
	...
	**/
	...
	public function index(){
		...
		$list = $m_event->where($where)->order($order)->page($p.','.$listrows)->select();
	...
	public function client_index(){
		...
		if(trim($_GET['act']) == 'excel'){
			if(vali_permission('event', 'export')){
				...
				$eventList = $m_event->where($where)->order($order)->limit($limit)->select();
				session('export_status', 1);
				$this->ajaxReturn(array('list'=>$eventList,'count'=>count($eventList)), '', 1);
// 				$this->excelExport($eventList);
			}else{
...
// 		$this->display();
	...
	public function excelExport($eventList=false){
		...
		$objProps->setTitle("crm Event Data");    
		$objProps->setSubject("crm Event Data");    
		$objProps->setDescription("crm Event Data");    
		$objProps->setKeywords("crm Event Data");    
        ...
        header("Content-Disposition:attachment;filename=crm_event_".date('Y-m-d',mktime())."_".$current_page.".xls");
