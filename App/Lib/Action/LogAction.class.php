...
class LogAction extends Action{
		...
		if($_POST['submit']){
			if($_POST['r']){
				...
				if($log_id = $m_log->add()){
					...
					$data[$m_id] = $model_id;
					if($m_r -> add($data)){
						if(in_array($module, array('leads', 'customer', 'business'))){
							...
							if($module == 'customer'){
								$save_array['update_time'] = time();
							}
							if($module == 'business'){
								M('business')->where("business_id =%d",$model_id)->setField('update_time',time());
							}
							if($save_array){
				...
				if($log_id = $m_log->add()){
					...
					if ($data['business_id']) {
						...
						M('business')->where("business_id =%d",$_POST['business_id'])->setField('update_time',time());
					}
					if ($data['task_id']) {
						M('RLogTask')->add($data);
					}
					...
					if ($data['customer_id']) {
						...
						M('customer')->where('customer_id =%d',$_POST['customer_id'])->setField('update_time',time());
					}
	...
	public function anly(){
		...
		if (session('?admin')){
			$all_ids = M('role')->where('user_id <> 0')->getField('role_id',true);
		}else{
