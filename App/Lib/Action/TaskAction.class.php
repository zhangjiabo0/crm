...
class TaskAction extends Action{
	...
	public function _initialize(){
		$action = array(
			...
			'allow'=>array('close','revert','open','changecontent','analytics','getcurrentstatus')
		);
	...
	public function edit(){
		...
		if($_POST['owner_name']){
			...
			if($is_updated){
				alert('success', L('MODIFY_TASK_SUCCESS'), U('task/index'));
			}else{
	...
	public function index(){
		...
		if(trim($_GET['act']) == 'excel'){	
			if(vali_permission('task', 'export')){
				...
				$taskList = $m_task->where($where)->order($order)->limit($limit)->select();	
		...
		$task_list = $m_task->where($where)->order($order)->page($p.','.$listrows)->select();
...
// 		$this->listrows = $listrows;
		$this->ajaxReturn(array('list'=>$task_list,'count'=>$count), '', 1);
// 		$this->assign('count',$count);
// 		$this->task_list = $task_list;
// 		$this->alert = parseAlert();
// 		$this->display();
			...
			if($owner_role_id || $about_roles || $res || session('?admin')){
				...
				if (in_array($task['owner_role_id'], getSubRoleId(false))) {
					if(!($task['comment_role_id'] > 0)){
						$comment_role_id = session('role_id');
					}
				}
				$this->comment_role_id = $comment_role_id;
	...
	**/
	public function open(){
		...
		if ($id >= 0) {
			...
			if ((is_array($task) && !empty($task))|| session('?admin')) {
			$res = $m_task->where('task_id = %d', $id)->setField('isclose', 0);			
				if($res){
	...
	public function excelExport($taskList=false){
		...
		$objProps->setTitle("crm Task Data");    
		$objProps->setSubject("crm Task Data");    
		$objProps->setDescription("crm Task Data");    
		$objProps->setKeywords("crm Task Data");    
		...
		foreach ($list as $k => $v) {
			$i++;
			$owner = D('RoleView')->where('role.role_id in (%s)', '0'.$v['owner_role_id'].'0')->select();
			$user_name = '';
			foreach($owner as $key=>$value){
				$user_name = $user_name.''.$value['user_name'].',';
			}
			$creator = D('RoleView')->where('role.role_id = %d', $v['creator_role_id'])->find();
			$objActSheet->setCellValue('A'.$i, $v['subject']);
			$objActSheet->setCellValue('B'.$i, $user_name);
        ...
        header("Content-Disposition:attachment;filename=crm_task_".date('Y-m-d',mktime())."_".$current_page.".xls");
