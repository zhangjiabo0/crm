	...
	public function client_index(){
		...
		if(trim($_GET['act']) == 'excel'){
			if(vali_permission('business', 'export')){
				...
				$businessList = $d_v_business->where($where)->order($order)->limit($limit)->select();
				session('export_status', 1);
				$this->ajaxReturn(array('list'=>$businessList,'count'=>count($businessList)), '', 1);
// 				$this->excelExport($businessList);
			}else{
...
// 		$this->display();
	...
	**/
	public function excelExport($businessList=false){
		...
		$objProps->setTitle("crm Business Data");    
		$objProps->setSubject("crm Business Data");    
		$objProps->setDescription("crm Business Data");    
		$objProps->setKeywords("crm Business Data");    
        ...
        header("Content-Disposition:attachment;filename=crm_business_".date('Y-m-d',mktime())."_".$current_page.".xls");
