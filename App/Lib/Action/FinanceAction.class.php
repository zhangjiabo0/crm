...
class FinanceAction extends Action{
	...
	public function index(){
		...
		switch ($this->type) {
			case 'receivables' :
				$receivables = D('ReceivablesView');
				$list = $receivables->order($order)->where($where)->page($p.','.$listrows)->select();
			...
			case 'receivingorder' :
				...
				$list = $receivingorder->order($order)->where($where)->page($p.','.$listrows)->select();
...
// 				$this->display('receivingorder');
			...
			case 'paymentorder' :
				...
				$this->listrows = $listrows;
// 				$this->alert = parseAlert();
				$this->ajaxReturn(array('list'=>$list,'count'=>$count,'money'=>$money,'sum_money'=>$sum_money), '', 1);
// 				$this->assign('page',$show);
// 				$this->assign('money',$money);
// 				$this->assign('sum_money',$sum_money);
// 				$this->assign('list',$list);
		...
		switch ($this->type) {
				...
				if($_POST['submit']){
					...
					if($id = $receivables->add($data)){
						if(intval($_POST['check_add_order']) == 1){
							$data_order['name'] = (trim($_POST['order_name']) && (trim($_POST['order_name']) != L('AUTOMATIC_GENERATION')))?trim($_POST['name']):'crm'.date('Ymd').mt_rand(1000,9999);
			...
			case 'receivingorder' :
				if($_POST['submit']){
					...
					$data['name'] = (trim($_POST['name']) && (trim($_POST['name']) != L('AUTOMATIC_GENERATION')))?trim($_POST['name']):'crm'.date('Ymd').mt_rand(1000,9999);
