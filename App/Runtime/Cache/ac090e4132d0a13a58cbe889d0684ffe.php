...
<div class="container">
	...
	<div class="page-header">
		<h4><?php echo L('EDIT THE ANNOUNCEMENT');?></h4>
	</div>
	<div class="row">
		<div class="span12">
				...
				<table class="table" border="0" cellspacing="1" cellpadding="0">
					...
					<tbody>
						...
						<tr>
							...
							<td><input type="text" name="title" class="span4" value="<?php echo ($announcement["title"]); ?>"/><?php echo L('100 CHARACTERS');?></td>
						</tr>
						<tr>
							<td class="tdleft"><?php echo L('HEADLINE COLOR');?></td>
							<td><input class="color" value="<?php echo ($announcement["color"]); ?>" name="color"/></td>
						</tr>
						<tr>
							<td class="tdleft"><?php echo L('WHETHER ISSUED');?></td>
							<td><input type="radio" <?php if($announcement['status'] == 1): ?>checked<?php endif; ?> value="1" name="status"/> <?php echo L('IS');?> &nbsp;
								<input type="radio"<?php if($announcement['status'] == 2): ?>checked<?php endif; ?> value="2" name="status"/> <?php echo L('ISNOT');?></td>
						</tr>
						<tr>
							<td class="tdleft"><?php echo L('WHETHER IN THE LOGIN SCREEN DISPLAY');?></td>
							<td><input type="radio" value="1" <?php if($announcement['isshow'] == 1): ?>checked<?php endif; ?> name="isshow"/> <?php echo L('IS');?> &nbsp;
								<input type="radio" value="0" <?php if($announcement['isshow'] == 0): ?>checked<?php endif; ?> name="isshow"/> <?php echo L('ISNOT');?></td>
						</tr>
						<tr>
							<td class="tdleft"><?php echo L('INFORM THE DEPARTMENT');?></td>
							<td><input type="checkbox" id="check_all"> &nbsp; <?php echo L('CHECK ALL');?> &nbsp;  &nbsp; <?php if(is_array($department_list)): $i = 0; $__LIST__ = $department_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input <?php echo ($vo['checked']); ?> name="announce_department[]" class="department" type="checkbox" value="<?php echo ($vo['department_id']); ?>"/> &nbsp; <?php echo ($vo['name']); ?> &nbsp;  &nbsp;<?php endforeach; endif; else: echo "" ;endif; ?></td>
						</tr>
						<tr>
							<td class="tdleft"><?php echo L('CONTENT');?></td>
							<td>
								<textarea name="content" id="content" style="width: 1000px; height: 350px;"><?php echo ($announcement["content"]); ?></textarea>
							</td>
						</tr>
					</tbody>
				</table>
