...
<div class="container">
	...
	<div class="row">
		<form action="<?php echo U('email/smtpdelete');?>" id="form1" method="post">
			<div class="span12">
				<p><div class="bulk-actions align-left">
					<a class="btn" href="javascript:void(0);" id="btn_delete"/><i class="icon-remove"></i> <?php echo L('DELETE');?></a>
					<div class="pull-right">
						<a href="javascript:void(0);" id="add" class="btn btn-primary"><i class="icon-plus"></i> <?php echo L('ADD_SEND_BOX');?></a>
					</div>
				</div></p>
			</div>
			<div class="span12">
				<table class="table table-hover table-striped table_thead_fixed" width="95%" border="0" cellspacing="1" cellpadding="0">
					<?php if(!empty($smtplist)): ?><thead>
							<tr>
								<th width="10%"><input type="checkbox" name="check_all" id="check_all" class="check_all"/> &nbsp;</th>
								<th width="20%"><?php echo L('SEND_BOX');?></th>
								<th width="20%"><?php echo L('EMAIL_ADDRESS');?></th>
								<th width="20%"><?php echo L('SMTP_SERVER_ADDRESS');?></th>
						...
						<tbody>
							<?php if(is_array($smtplist)): $i = 0; $__LIST__ = $smtplist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
									<td><input type="checkbox" class="list" name="smtp_id[]" value="<?php echo ($vo["smtp_id"]); ?>"/>
									</td>
									<td><?php echo ($vo["name"]); ?></td>
									<td><?php echo ($vo["smtp"]["MAIL_ADDRESS"]); ?></td>
									<td><?php echo ($vo["smtp"]["MAIL_SMTP"]); ?></td>
									<td><a href="javascript:void(0);" rel="<?php echo ($vo['smtp_id']); ?>" class="edit">编辑</a></td>
...
$(function(){
	...
	$("#add").click(function(){
		...
		$('#dialog-add').load('<?php echo U("email/smtpadd");?>');
	});
	$(".edit").click(function(){
		var id = $(this).attr('rel');
		$('#dialog-edit').dialog('open');
		$('#dialog-edit').load('<?php echo U("email/smtpedit","id");?>'+id);
	});
	$("#sort_btn").click(
		function() {
			position = [];
