...
.ztree li span.button.add {margin-left:2px; margin-right: -1px; background-position:-144px 0; vertical-align:top; *vertical-align:middle}
	...
	<div id="dialog-message1" class="hide" title="<?php echo L('EDIT_DEPARTMENT_INFO');?>">loading...</div>	
	<div id="dialog-message4" class="hide" title="<?php echo L('EDIT_POSITION_INFO');?>">loading...</div>	
	<div id="dialog-message2" class="hide" title="<?php echo L('DIALOG_ADD_DEPARTMENT');?>">loading...</div>	
...
<script type="text/javascript">
	$(function(){
		...
		$(".department_edit").click(function(){
			...
			id = $(this).attr('rel');
			$('#dialog-message1').load('<?php echo U("user/department_edit","id=");?>'+id);
		});
		$(".position_edit").click(function(){
			$('#dialog-message4').dialog('open');
			id = $(this).attr('rel');
			$('#dialog-message4').load('<?php echo U("user/roleedit","id=");?>'+id);
		});
		$(".department_delete").click(function(){
			if(confirm("<?php echo L('CONFIRM_TO_DELETE_DEPARTMENT');?>")){
				id = $(this).attr('rel');
				window.location="<?php echo U('user/department_delete','id=');?>"+id;
			}
		});
	...
	function deleteDepartment(id,name){
		var v = confirm("<?php echo L('CONFIRM_TO_DELETE_DEPARTMENT_PARAM');?>");
		if(v == true){
			window.location="<?php echo U('User/department_delete','id=');?>"+id;
		}
	}
	$('#department_add').click(function () { 
		...
		$('#dialog-message').load('<?php echo U('User/department_add');?>');
		return false;
	});
